...
export class FormComponent  implements OnInit {
  ...
  const alert = await this.alertController.create({
    header: 'Coinciden',
    subHeader: "Usuario:"+this.registroForm.value.usuario+"\n"+"Contraseñas: "+this.registroForm.value.password+" - "+this.registroForm.value.confirmcontr,
...
async AlertaMala() {
  ...
  coinciden(){
    console.log(this.registroForm.value.confirmcontr + " - " +this.registroForm.value.password)
    if(this.registroForm.value.password == this.registroForm.value.confirmcontr){
      this.Alerta();
    }else{
      this.AlertaMala();
    }
